...
###########################
print('\033[1m Prepare the data for the least-squares learning procedure for the state \033[0m')
...
print('\033[1m Prepare the data for the least-squares learning procedure for the output \033[0m')
Xhatmax = np.load(Xhat_file)
		...
		if not is_nan:
			for n, alpha_out_lin in enumerate(gamma_reg_lin):
				for m, alpha_out_quad in enumerate(gamma_reg_quad): 
					...
					print('\033[1m Calculations for regularization parameters {}, {} \033[0m'.format(alpha_out_lin, alpha_out_quad))
					...
					mean_err_Gamma_n_train 	= np.abs(mean_Gamma_n_ref - mean_Gamma_n_OpInf_train)/mean_Gamma_n_ref
					std_err_Gamma_n_train 	= np.abs(std_Gamma_n_ref - std_Gamma_n_OpInf_train)/std_Gamma_n_ref

					mean_err_Gamma_c_train 	= np.abs(mean_Gamma_c_ref - mean_Gamma_c_OpInf_train)/mean_Gamma_c_ref
					std_err_Gamma_c_train 	= np.abs(std_Gamma_c_ref - std_Gamma_c_OpInf_train)/std_Gamma_c_ref

					
					mean_err_Gamma_n_pred 	= np.abs(mean_Gamma_n_ref - mean_Gamma_n_OpInf_pred)/mean_Gamma_n_ref
					std_err_Gamma_n_pred 	= np.abs(std_Gamma_n_ref - std_Gamma_n_OpInf_pred)/std_Gamma_n_ref

					mean_err_Gamma_c_pred 	= np.abs(mean_Gamma_c_ref - mean_Gamma_c_OpInf_pred)/mean_Gamma_c_ref
					std_err_Gamma_c_pred 	= np.abs(std_Gamma_c_ref - std_Gamma_c_OpInf_pred)/std_Gamma_c_ref


					if mean_err_Gamma_n_train < prec_mean and std_err_Gamma_n_train < prec_std and mean_err_Gamma_c_train < prec_mean and std_err_Gamma_c_train < prec_std and \
					   mean_err_Gamma_n_pred < prec_mean and std_err_Gamma_n_pred < prec_std and mean_err_Gamma_c_pred < prec_mean and std_err_Gamma_c_pred < prec_std:
					...
					print('\033[1m Training errors for Gamma n: {}, {} \033[0m'.format(mean_err_Gamma_n_train, std_err_Gamma_n_train))
					print('\033[1m Training errors for Gamma c: {}, {} \033[0m'.format(mean_err_Gamma_c_train, std_err_Gamma_c_train))

					print('\033[1m Prediction errors for Gamma n: {}, {} \033[0m'.format(mean_err_Gamma_n_pred, std_err_Gamma_n_pred))
					print('\033[1m Prediction errors for Gamma c: {}, {} \033[0m'.format(mean_err_Gamma_c_pred, std_err_Gamma_c_pred))
...
np.savez('results/Gamma_ensemble_statistics_c1_5.0_training_end' + str(training_size) + '_r'+str(r)+'.npz', \
        Gamma_n_mean=Gamma_n_mean, Gamma_n_std=Gamma_n_std, Gamma_c_mean=Gamma_c_mean, Gamma_c_std=Gamma_c_std)

np.savez('/storage1/HW/paper/res_c1_5.0/Gamma_ensemble_c1_5.0_training_end' + str(training_size) + '_r'+str(r)+'.npz', \
        Gamma_n_ensemble=Gamma_n_ensemble, Gamma_c_ensemble=Gamma_c_ensemble)

np.savez('/storage1/HW/paper/res_c1_5.0/reg_params_ensemble_c1_5.0_training_end' + str(training_size) + '_r'+str(r)+'.npz', \
        alphas_lin=alphas_lin, alphas_quad=alphas_quad)
