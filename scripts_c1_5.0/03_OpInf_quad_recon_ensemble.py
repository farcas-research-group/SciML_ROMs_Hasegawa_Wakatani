...
data = np.load('/storage1/HW/paper/res_c1_5.0/reg_params_ensemble_c1_5.0_training_end' + str(training_size) + '_r'+str(r)+'.npz')
...
alphas_state = np.unique(np.vstack(([alphas_lin_state.T], [alphas_quad_state.T])).T, axis=0)

alphas_lin_output 	= alphas_lin[:, 1]
alphas_quad_output 	= alphas_quad[:, 1]

alphas_output = np.unique(np.vstack(([alphas_lin_output.T], [alphas_quad_output.T])).T, axis=0)
...
for alpha_pair_state in alphas_state:
	...
	if not is_nan:
		for alpha_pair_output in alphas_output:
			...
			mean_err = np.mean([mean_err_Gamma_n_train, std_err_Gamma_n_train, mean_err_Gamma_c_train, std_err_Gamma_c_train])
...
np.savez('/storage1/HW/paper/res_c1_5.0/postprocessing_ensemble_c1_5.0_training_end' + str(training_size) + '_r'+str(r)+'.npz', \
	        red_op_lin_state=A, X_OpInf=X_OpInf_full, Gamma_n_pred=ts_Gamma_n, Gamma_c_pred=ts_Gamma_c)
