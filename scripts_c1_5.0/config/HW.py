...
def rescale(X,Xm,td):
    ...
    for i in range(dw):
        ...
        X[i*n:(i+1)*n,:]    = Xm[i*n : (i+1)*n][:,np.newaxis] + X[i*n : (i+1)*n,:]*scl
...
ridge_alf_lin_all   = np.logspace(1, 4., num = 10)
ridge_alf_quad_all  = np.logspace(10., 14., num = 10)

gamma_reg_lin   = np.logspace(-4, 2, 20)
gamma_reg_quad  = np.logspace(-4, 2, 20)

time_steps_rec = [999, 1999, 3999, 5999, 7999, 9999, 10000, 11999, 12999, 13999, 14999, 15999, 16999, 17999, 18999, 20000]
