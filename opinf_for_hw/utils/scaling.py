...
def minus1to1(X, axis=None, bounds=None, rev=False):
    if rev:
        if len(bounds.shape) == 2:
            ...
            minX = bounds[0]
            maxX = bounds[1]
        X, shf, scl = shiftthenscale(
            X, shf=-(maxX + minX) / (maxX - minX), scl=2.0 / (maxX - minX), axis=axis
        )
    else:
        if bounds is None:
            minX = np.min(X, axis)
            ...
            minX = bounds[0]
            maxX = bounds[1]
        X, scl, shf = scalethenshift(
            X, scl=0.5 * (maxX - minX), shf=(maxX + minX) / (maxX - minX), axis=axis
        )
    return X, scl, shf
...
def zeroto1(X, axis=None, bounds=None):
    if bounds is None:
        ...
        maxX = bounds[1]
    X, scl, shf = scalethenshift(
        X, scl=maxX - minX, shf=minX / (maxX - minX), axis=axis
    )
    return X, scl, shf
