...
def read_raw_snapshots(
    ...
    for iter_count, iter_num in enumerate(iterator):
        ...
        data_set = tec.data.load_tecplot_szl(
            [mesh_loc, data_loc(iter_num)], read_data_option=2
        )  # data_set is overarching data object
        ...
        if iter_num == snapshot_time_start:
            ...
            snapshot_mat = np.zeros(
                (n_cells * (len(primitive_var_list)), n_snapshots), dtype=np.float64
            )
...
def lift_snapshot_matrix(
    data_loc, mesh_loc, snapshot_time_start, snapshot_time_end, primitive_var_list
):
    raw_snapshots, n_cells = read_raw_snapshots(
        data_loc, mesh_loc, snapshot_time_start, snapshot_time_end, primitive_var_list
    )
...
def lift_and_normalize_snapshot_matrix(
    data_loc,
    mesh_loc,
    snapshot_time_start,
    snapshot_time_end,
    ...
    lifted_snapshots, n_cells = lift_snapshot_matrix(
        data_loc, mesh_loc, snapshot_time_start, snapshot_time_end, primitive_var_list
    )
    ...
    lifted_and_normalized_snapshots = np.concatenate(
        (xi_n, u_n, v_n, w_n, p_n, rZ_n, rC_n), axis=0
    )
