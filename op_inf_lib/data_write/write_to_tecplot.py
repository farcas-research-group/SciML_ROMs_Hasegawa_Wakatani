...
def get_tecplot_setup(mesh_loc, available_datafile, addtl_vars, ignore_vars=False, vars_to_ignore=None, out_dtype=2):

    data_set 	= tec.data.load_tecplot_szl([mesh_loc, available_datafile], read_data_option=2)
    ...
    if ignore_vars:
        ...
        for var in varList_cc:
            data_set.add_variable(var, dtypes=tec.constant.FieldDataType(out_dtype), locations=tec.constant.ValueLocation(0))
...
def write_data_to_szplt_file(varList_cc, data_set, ex_zone, n_dims, rom_data, t, dt, out_file):
