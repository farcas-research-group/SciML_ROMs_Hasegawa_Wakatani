		...
		if not is_nan:
			for n, alpha_out_lin in enumerate(gamma_reg_lin):
				for m, alpha_out_quad in enumerate(gamma_reg_quad): 
					...
					err_Gamma_n_train 	= np.linalg.norm(ts_Gamma_n[:training_end] - Gamma_n)/np.linalg.norm(Gamma_n)
					err_Gamma_c_train 	= np.linalg.norm(ts_Gamma_c[:training_end] - Gamma_n)/np.linalg.norm(Gamma_c)
					...
					print('\033[1m Average training error: {} \033[0m'.format(avg_err_train))
					print('\033[1m Best training error: {} \033[0m'.format(avg_err_best))
...
np.savez('results/bad_reg_params_c1_0.1_training_end' + str(training_size) + '_r'+str(r)+'.npz', \
        alpha_state_lin_best=alpha_state_lin_best, alpha_state_quad_best=alpha_state_quad_best, \
        alpha_out_lin_best=alpha_out_lin_best, alpha_out_quad_best=alpha_out_quad_best)